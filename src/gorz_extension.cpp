...
struct GorGlobalState : public GlobalTableFunctionState {
    ...
    idx_t MaxThreads() const override {
        if (kind == GorKind::GORD) return std::max<idx_t>(groups.size(), 1);
        if (gorzParallel) return std::max<idx_t>(byteRanges.size(), 1);
...
std::string resolvePathViaFileSearch(ClientContext &context, const std::string &p) {
    ...
    Value v;
    if (!context.TryGetCurrentSetting("file_search_path", v)) return p;
    std::string searchPath;
    try { searchPath = v.GetValue<std::string>(); }
    catch (...) { return p; }
    if (searchPath.empty()) return p;
    ...
    while (start <= searchPath.size()) {
        ...
        std::string entry = searchPath.substr(start,
            comma == std::string::npos ? searchPath.size() - start : comma - start);
        if (!entry.empty()) {
...
public:
    explicit FileHandleStreamBuf(unique_ptr<FileHandle> handle,
                                 std::size_t bufSize = 1u << 16)
...
protected:
    int_type underflow() override {
        ...
        const int64_t n = handle_->Read(buffer_.data(),
                                        static_cast<idx_t>(buffer_.size()));
        if (n <= 0) {
    ...
    pos_type seekoff(off_type off, std::ios_base::seekdir dir,
        ...
        switch (dir) {
            ...
            case std::ios_base::cur:
                ...
                break;
            case std::ios_base::end: target = fileSize_ + off; break;
...
public:
    explicit FileHandleInputStream(unique_ptr<FileHandle> handle)
        : std::istream(nullptr), buf_(std::move(handle)) {
        rdbuf(&buf_);
    }
...
public:
    explicit FileHandleOutStreamBuf(unique_ptr<FileHandle> handle)
...
public:
    explicit FileHandleOutputStream(unique_ptr<FileHandle> handle)
        : std::ostream(nullptr), buf_(std::move(handle)) {
        rdbuf(&buf_);
    }
...
std::unique_ptr<std::istream> openViaFileSystem(ClientContext &context,
...
LogicalType resolveColumnType(std::size_t idx,
    ...
    if (schema.has_value() && schema->size() == columnNames.size()) {
        ...
        if (entry.has_value()) {
            switch (*entry) {
                case gorz::GorColumnType::String:  return LogicalType::VARCHAR;
                case gorz::GorColumnType::Integer: return LogicalType::INTEGER;
                case gorz::GorColumnType::Long:    return LogicalType::BIGINT;
                case gorz::GorColumnType::Double:  return LogicalType::DOUBLE;
            }
        }
    }
    if (idx == 1) return LogicalType::INTEGER;  // gor convention: pos
...
bool tryExtractChromOrPos(const Expression &e, SeekHint &hint,
                          const std::vector<idx_t> &boundIdxToTableCol) {
    if (e.expression_class != ExpressionClass::BOUND_COMPARISON) return false;
    ...
    if (colExpr->expression_class != ExpressionClass::BOUND_COLUMN_REF) return false;
    if (constExpr->expression_class != ExpressionClass::BOUND_CONSTANT) return false;
    ...
    if (boundIdx >= boundIdxToTableCol.size()) return false;
    const idx_t col = boundIdxToTableCol[boundIdx];
    if (constRef.value.IsNull()) return false;

    // Reflect the comparison when we swapped sides: `5 < pos` ≡ `pos > 5`.
    if (flipped) {
        switch (cmpType) {
            case ExpressionType::COMPARE_GREATERTHAN:        cmpType = ExpressionType::COMPARE_LESSTHAN; break;
            case ExpressionType::COMPARE_GREATERTHANOREQUALTO: cmpType = ExpressionType::COMPARE_LESSTHANOREQUALTO; break;
            case ExpressionType::COMPARE_LESSTHAN:           cmpType = ExpressionType::COMPARE_GREATERTHAN; break;
            case ExpressionType::COMPARE_LESSTHANOREQUALTO:  cmpType = ExpressionType::COMPARE_GREATERTHANOREQUALTO; break;
            case ExpressionType::COMPARE_EQUAL:              break;  // symmetric
    ...
    if (col == 0) {
        ...
        if (cmpType != ExpressionType::COMPARE_EQUAL) return false;
        try {
        ...
        try {
            ...
            switch (cmpType) {
                case ExpressionType::COMPARE_EQUAL:
                    ...
                    hint.haveLo = hint.haveHi = true; return true;
                case ExpressionType::COMPARE_GREATERTHAN:
                    hint.posLo = std::max(hint.posLo, v + 1); hint.haveLo = true; return true;
                case ExpressionType::COMPARE_GREATERTHANOREQUALTO:
                    hint.posLo = std::max(hint.posLo, v); hint.haveLo = true; return true;
                case ExpressionType::COMPARE_LESSTHAN:
                    hint.posHi = std::min(hint.posHi, v - 1); hint.haveHi = true; return true;
                case ExpressionType::COMPARE_LESSTHANOREQUALTO:
                    hint.posHi = std::min(hint.posHi, v); hint.haveHi = true; return true;
                default: return false;
            }
        } catch (...) { return false; }
    }
...
bool tryExtractBetween(const Expression &e, SeekHint &hint,
                       const std::vector<idx_t> &boundIdxToTableCol) {
    if (e.expression_class != ExpressionClass::BOUND_BETWEEN) return false;
    ...
    const idx_t boundIdx = colRef.binding.column_index;
    if (boundIdx >= boundIdxToTableCol.size()) return false;
    if (boundIdxToTableCol[boundIdx] != 1) return false;  // pos only

    auto extractEnd = [&](const Expression &endExpr, bool inclusive, bool isLower) {
        if (endExpr.expression_class != ExpressionClass::BOUND_CONSTANT) return;
        auto &c = endExpr.Cast<BoundConstantExpression>();
        if (c.value.IsNull()) return;
        try {
...
void walkForSeekHint(const Expression &e, SeekHint &hint,
                     const std::vector<idx_t> &boundIdxToTableCol) {
    ...
    if (tryExtractBetween(e, hint, boundIdxToTableCol)) return;
...
void seekHintComplexPushdown(ClientContext &, LogicalGet &get, FunctionData *bind_data,
    ...
    for (auto &expr : filters) {
        if (expr) walkForSeekHint(*expr, bind.seek, boundIdxToTableCol);
    }
}
...
void collectTagParam(TableFunctionBindInput &input, const char *name,
                     std::vector<std::string> &out) {
    auto it = input.named_parameters.find(name);
    if (it == input.named_parameters.end() || it->second.IsNull()) return;
    const Value &v = it->second;
    if (v.type().id() == LogicalTypeId::LIST) {
        for (const auto &child : ListValue::GetChildren(v)) {
            if (!child.IsNull()) out.push_back(child.ToString());
        }
    } else {
...
unique_ptr<FunctionData> ReadGorBindImpl(ClientContext &context, TableFunctionBindInput &input,
    ...
    GorKind kind = forced.has_value()
        ? *forced
        : (StringUtil::EndsWith(StringUtil::Lower(path), ".gord") ? GorKind::GORD
                                                                  : GorKind::GORZ);
    ...
    if (kind == GorKind::GORZ) {
        ...
        try {
            ...
            throw IOException(std::string("read_gor: format error in ") + path
                + ": " + e.what());
        } catch (const std::exception &e) {
    ...
    try {
        ...
        if (!data->tagFilter.empty() && !gord.hasTags) {
            throw BinderException(
                "read_gor: f / ff given but dictionary '" + path + "' carries no tags");
        }
...
ReadGorInitGlobal(ClientContext &context, TableFunctionInitInput &input) {
    ...
    if (bind.kind == GorKind::GORD) {
        ...
        if (!survivors.empty()) {
            ...
            idx_t g = std::min<idx_t>(survivors.size(),
                                      std::max<idx_t>(1, static_cast<idx_t>(nThreads)));
            ...
            for (idx_t start = 0; start < survivors.size(); start += chunk) {
                ...
                state->groups.emplace_back(survivors.begin() + start,
                                           survivors.begin() + end);
            }
        }
    ...
    if (!hasSeek) {
        ...
        if (g > 1) {
            ...
            for (idx_t i = 0; i < g; ++i) {
                ...
                int64_t e = (i + 1 == g) ? fileSize
                                         : bodyStart + static_cast<int64_t>(i + 1) * chunk;
...
void storeField(DataChunk &output, idx_t col, idx_t row,
                const LogicalType &type, std::string_view field) {
    switch (type.id()) {
        case LogicalTypeId::VARCHAR:
            FlatVector::GetData<string_t>(output.data[col])[row] =
                StringVector::AddString(output.data[col],
...
ReadGorInitLocal(ExecutionContext &, TableFunctionInitInput &,
...
void ReadGorExecute(ClientContext &, TableFunctionInput &data,
                    DataChunk &output) {
    ...
    if (bind.kind == GorKind::GORD) {
        ...
        while (row < capacity) {
            if (!local.reader) break;  // no group / all groups drained
            if (!local.reader->nextRow(line)) {
                if (!claimNextGroup(g, local)) break;
                continue;
            }
            splitTabs(line, fields);
            if (fields.size() < nCols) fields.resize(nCols, std::string_view());
            storeProjected();
        }
    ...
    if (g.gorzParallel) {
        auto &local = data.local_state->Cast<GorLocalState>();
        while (row < capacity) {
            if (!local.gorzReader) break;
            if (!local.gorzReader->nextRow(line)) {
                if (!claimNextByteRange(g, local)) break;
                continue;
            }
            splitTabs(line, fields);
            if (fields.size() < nCols) fields.resize(nCols, std::string_view());
    ...
    while (row < capacity) {
        ...
        if (g.rangeActive) {
            ...
            if (fields.size() > 1 && !fields[1].empty()) {
                auto r = std::from_chars(fields[1].data(),
                                         fields[1].data() + fields[1].size(),
                                         rowPos);
                if (r.ec != std::errc()) rowPos = 0;
            }
            if (g.haveChromEq) {
                if (rowChrom != g.chromEq) {
                    if (rowChrom > g.chromEq) { g.exhausted = true; break; }
                    continue;  // before our chrom; keep walking
                }
                if (rowPos > g.posHi) { g.exhausted = true; break; }
            } else {
                if (rowPos > g.posHi || rowPos < g.posLo) continue;
            }
            if (g.haveChromEq && rowPos < g.posLo) continue;
        }
        storeProjected();
    }
    output.SetCardinality(row);
...
unique_ptr<FunctionData> GorCopyBind(ClientContext &, CopyFunctionBindInput &,
                                     const vector<string> &names,
                                     const vector<LogicalType> &sqlTypes,
                                     GorWriteFormat fmt) {
    const char *fmtName = (fmt == GorWriteFormat::GORZ) ? "gorz" : "gor";
    if (names.size() < 2) {
        throw BinderException(
            "COPY ... TO (FORMAT %s): need at least 2 columns (chrom, pos); got %llu",
            fmtName, (unsigned long long)names.size());
    }
    if (sqlTypes[0].id() != LogicalTypeId::VARCHAR) {
        throw BinderException(
            "COPY ... TO (FORMAT %s): first column '%s' (chrom) must be VARCHAR, got %s",
            fmtName, names[0].c_str(), sqlTypes[0].ToString().c_str());
    }
    if (!sqlTypes[1].IsIntegral()) {
        throw BinderException(
            "COPY ... TO (FORMAT %s): second column '%s' (pos) must be an integer type, got %s",
...
unique_ptr<FunctionData> GorzCopyBind(ClientContext &ctx, CopyFunctionBindInput &in,
                                      const vector<string> &names,
                                      const vector<LogicalType> &types) {
    return GorCopyBind(ctx, in, names, types, GorWriteFormat::GORZ);
}
unique_ptr<FunctionData> GorPlainCopyBind(ClientContext &ctx, CopyFunctionBindInput &in,
                                          const vector<string> &names,
                                          const vector<LogicalType> &types) {
    return GorCopyBind(ctx, in, names, types, GorWriteFormat::GOR);
}

unique_ptr<GlobalFunctionData> GorCopyInitGlobal(ClientContext &context,
    ...
    unique_ptr<FileHandle> handle;
    try {
        handle = fs.OpenFile(file_path, FileOpenFlags::FILE_FLAGS_WRITE |
                                            FileOpenFlags::FILE_FLAGS_FILE_CREATE);
    } catch (const std::exception &e) {
...
void GorCopySink(ExecutionContext &, FunctionData &bind_data, GlobalFunctionData &gstate,
    ...
    for (idx_t r = 0; r < nrow; ++r) {
            ...
            if (chrom == gs.lastChrom) {
                if (pos < gs.lastPos) {
                    throw InvalidInputException(
                        "Wrong order in gor stream " + std::to_string(pos) + " " +
                        std::to_string(gs.lastPos) + " (need ORDER BY chrom, pos)");
                }
            } else if (chrom < gs.lastChrom) {
                throw InvalidInputException(
                    "Wrong chromosome order in gor stream " + chrom + " " + gs.lastChrom +
        ...
        for (idx_t c = 2; c < ncol; ++c) {
            if (c > 2) rest.push_back('\t');
            Value v = input.GetValue(c, r);
            if (!v.IsNull()) rest += v.ToString();
        }
...
void GorCopyCombine(ExecutionContext &, FunctionData &, GlobalFunctionData &,
                    LocalFunctionData &) {
...
CopyFunctionExecutionMode GorCopyExecMode(bool /*preserve_insertion_order*/,
...
void LoadInternal(ExtensionLoader &loader) {
    ...
    auto registerGor = [&](const char *name,
                           duckdb::table_function_bind_t bindFn) {
        duckdb::TableFunction tf(name, {duckdb::LogicalType::VARCHAR},
                                 duckdb::ReadGorExecute, bindFn,
    ...
    auto registerCopy = [&](const char *name, const char *ext,
                            duckdb::copy_to_bind_t bindFn) {
    ...
    loader.SetDescription(
        "Read/write GORpipe .gorz files and .gord dictionaries as native DuckDB "
        "tables (read_gor / read_gorz / read_gord + COPY TO). "
        "https://github.com/gorfather/duckdb-gorz");
}

void GorzExtension::Load(ExtensionLoader &loader) { LoadInternal(loader); }
std::string GorzExtension::Name() { return "gorz"; }
